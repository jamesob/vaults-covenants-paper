...
\begin{abstract}
  ...
  would be possible only with proposed consensus changes (e.g. \ctv{}). 
...
policies. But in the opinion of the author, these advanced features remain niche and
...
\begin{enumerate}
  \item a witness program (\spk{}) that, when hashed with \code{sha256d}, yields the
    \code{<unvault-pk-hash>}, and
...
and also removes the need for an additional transaction -- which may onerous when 
...
only important as blockspace becomes scarcer; the ability to batch may prevent a
situation in which, given a large number of vaults and a low timeout period, a custodian
otherwise may not be able to get all the confirmations needed to thwart an attacker.
